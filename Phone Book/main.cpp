...
void add_contact(){ //adds a person 
    ...
    bool good_case = false;   //for address error check
    ...
    while(temp_num.length() != 10){ //check for correct size
        ...
        getline(cin, temp_num); //get again
        
    }
    while(!(isdigit(temp_num.at(0)) && isdigit(temp_num.at(1)) && isdigit(temp_num.at(2)) && isdigit(temp_num.at(3)) && isdigit(temp_num.at(4)) && isdigit(temp_num.at(5)) && isdigit(temp_num.at(6)) && isdigit(temp_num.at(7)) && isdigit(temp_num.at(8)) && isdigit(temp_num.at(9)))){ //check
        cout<<"  -> Only numbers allowed. Try again: "; //err msg
        getline(cin, temp_num); //get again
    }
    ...
    while(!good_case){ //while bad input
        ...
        for(int i = 0; i<temp_add.length(); ++i){ //for each char
            ...
            good_case = true; //if passed those checks then true
        }
        if(!good_case){ //if bad
            cout<<"  -> Bad format. Try again: "; //err msg
            getline(cin, temp_add); //get 
        }
    }
    ...
    while(temp_pc.length() != 6){ //check size
        cout<<"  -> Postal code is in wrong format. Try again: "; //err msg
        getline(cin, temp_pc); //get again
    }
    while(!(isalpha(temp_pc.at(0)) && isdigit(temp_pc.at(1)) && isalpha(temp_pc.at(2)) && isdigit(temp_pc.at(3)) && isalpha(temp_pc.at(4)) && isdigit(temp_pc.at(5)))){ //check 
        cout<<"  -> Has to be letter, number, letter, etc. Try again: "; //err message
        getline(cin, temp_pc); //get again
    }
...
void edit_contact(){
    ...
    while(true){
        ...
        if(edit_choice == 0){ //NAME
            ...
            while(!(isdigit(temp_num.at(0)) && isdigit(temp_num.at(1)) && isdigit(temp_num.at(2)) && isdigit(temp_num.at(3)) && isdigit(temp_num.at(4)) && isdigit(temp_num.at(5)) && isdigit(temp_num.at(6)) && isdigit(temp_num.at(7)) && isdigit(temp_num.at(8)) && isdigit(temp_num.at(9)))){ //check
                cout<<"  -> Only numbers allowed. Try again: "; //err ms
                getline(cin, temp_num); //get again
            }
            ...
            while(!good_case){ //while bad input
                ...
                for(int i = 0; i<temp_add.length(); ++i){ //for each char
                    ...
                    if(!found_space){ //has to be digit
                        if(!isdigit(temp_add.at(i))){ //if not digit
                            good_case = false; //bad
                            break; //get out of loop
                        }
                    }else{ //has to be alpha or a space
                        if(!(isalpha(temp_add.at(i)) || temp_add.at(i) == ' ')){ //if digit
                            good_case = false; //bad
                            break; //get out of loop
                        }
                    }
                    good_case = true; //if all checks passed
                }
                if(!good_case){ //if bad
                    cout<<"  -> Bad format. Try again: "; //err msg
                    getline(cin, temp_add); //get again
                }
            }
            ...
            while(!(isalpha(temp_pc.at(0)) && isdigit(temp_pc.at(1)) && isalpha(temp_pc.at(2)) && isdigit(temp_pc.at(3)) && isalpha(temp_pc.at(4)) && isdigit(temp_pc.at(5)))){ //check
                cout<<"  -> Has to be letter, number, letter, etc. Try again: "; //err msg
                getline(cin, temp_pc); //get again
            }
